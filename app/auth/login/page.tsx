    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.includes("400") || error.status === 400) {
          setError(
            "구글 로그인 설정이 완료되지 않았습니다. Supabase 대시보드에서 Google OAuth를 설정해주세요. 자세한 내용은 SUPABASE_SETUP.md를 참고하세요."
          );
        } else {
      ...
      setError(
        error.message ||
          "구글 로그인 중 오류가 발생했습니다. Supabase에서 Google OAuth가 활성화되어 있는지 확인해주세요."
      );
      setLoading(false);
    }
  };
